...
def preprocess_network_data(df, scaler=None, fit_scaler=True, expected_columns=None):
    features = df[['bytes_sent', 'bytes_received', 'duration', 
                  'port', 'protocol_type', 'service', 'flag']].copy()
    categorical_cols = ['protocol_type', 'service', 'flag']
    features = pd.get_dummies(features, columns=categorical_cols)
    
    # Handle matching columns to training set if expected_columns is provided
...
def train_network_anomaly_models(X_train):
    ...
    # Calculate threshold based on 99th percentile of reconstruction error on training data
    predictions = autoencoder.predict(X_train)
    if predictions.shape != X_train.shape:
        predictions = predictions.reshape(X_train.shape)
    mse = np.mean(np.power(X_train - predictions, 2), axis=1)
    threshold = np.percentile(mse, 99)
    
    return autoencoder, iso_forest, threshold

def detect_network_anomalies(autoencoder, iso_forest, X_test, threshold):
    ...
    # Set anomaly threshold using the trained threshold
...
class CybersecurityThreatDetector:
    def __init__(self):
        ...
        self.network_threshold = None
        self.network_feature_cols = None
    ...
    def train(self, train_network=True, train_malware=True, train_phishing=True):
        ...
        if train_network:
            ...
            features, self.network_scaler = preprocess_network_data(network_data, fit_scaler=True)
            self.network_feature_cols = list(features.columns)
            X_train, X_test = train_test_split(features, test_size=0.3, random_state=42)
            self.network_autoencoder, self.network_isoforest, self.network_threshold = train_network_anomaly_models(X_train)
        if train_malware:
    ...
    def save_models(self, directory="./models"):
        ...
        joblib.dump(self.network_threshold, f"{directory}/network_threshold.pkl")
        joblib.dump(self.network_feature_cols, f"{directory}/network_feature_cols.pkl")
    ...
    def load_models(self, directory="./models"):
        try:
            ...
            self.network_threshold = joblib.load(f"{directory}/network_threshold.pkl")
            self.network_feature_cols = joblib.load(f"{directory}/network_feature_cols.pkl")
    ...
    def detect_network_threats(self, network_data):
        ...
        if isinstance(network_data, pd.DataFrame) and 'protocol_type' in network_data.columns:
            processed_data, _ = preprocess_network_data(
                network_data, 
                scaler=self.network_scaler, 
                fit_scaler=False, 
                expected_columns=self.network_feature_cols
            )
        else:
            processed_data = network_data
        anomalies, scores = detect_network_anomalies(
            self.network_autoencoder, self.network_isoforest, processed_data, self.network_threshold
        )
        return {
            'anomalies': anomalies,
            'scores': scores,
    ...
    def detect_malware(self, file_paths):
        ...
        for file_path in file_paths:
            try:
                ...
                if hasattr(self.malware_detector, 'feature_names_in_'):
                    features_df = features_df[self.malware_detector.feature_names_in_]
...
if __name__ == "__main__":
    ...
    network_results = detector.detect_network_threats(test_network_data)
    print(f"Network traffic analysis:")
    print(f"  - Total traffic flows: {len(test_network_data)}")
